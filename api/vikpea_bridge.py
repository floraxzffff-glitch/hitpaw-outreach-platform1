...
import contact_threshold_config  # noqa: E402
...
def get_confirmed_candidates() -> List[Dict[str, Any]]:
    ...
    for candidate in candidates:
        ...
        candidate["_contacted_note"] = ""  # 新增：联络历史标注
        ...
        # 5. 检查已联络历史
        threshold_days = contact_threshold_config.get_contact_threshold()
        contacted_exclude, contacted_days, contacted_note = config.check_contacted_history(
            channel_name, email, threshold_days=threshold_days
        )
        if contacted_days is not None:
            if contacted_exclude:
                ...
                candidate["_contacted_note"] = f"{contacted_days}天前已联络"
            if contacted_note:
                candidate["_contacted_note"] += f" - {contacted_note}"
...
def get_pending_candidates() -> List[Dict[str, Any]]:
    ...
    for candidate in candidates:
        channel_name = str(candidate.get("频道名", ""))
        email = str(candidate.get("人工确认邮箱", "") or candidate.get("候选邮箱", ""))

        candidate["_contacted_note"] = ""
        ...
        if contacted_days is not None:
            if contacted_exclude:
                ...
                candidate["_contacted_note"] = f"⚠️ {contacted_days}天前已联络"
            if contacted_note:
                candidate["_contacted_note"] += f" - {contacted_note}"
